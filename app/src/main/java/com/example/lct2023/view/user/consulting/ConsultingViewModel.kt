...
import com.example.lct2023.view.util.DropDownItem
...
data class Kno(
    ...
    val controlList: List<ControlType>
): DropDownItem

data class ControlType(
    override val name: String,
    val themeList: List<ConsultTheme>
): DropDownItem
...
class ConsultingViewModel @Inject constructor(
    ...
    private val _state: MutableStateFlow<ViewStateClass<List<Kno>>> = MutableStateFlow(
        ViewStateClass.Loading)
    val state = _state.asStateFlow()
    val slots = mockSlots()
    val slotsOrder = slots.keys.map { it.take(2).toInt() to it }.sortedBy { it.first }
...
fun mockSlots(): Map<String, List<Slot>> {
    ...
    val answer = slots.groupBy { format.format(it.date)}

    return answer.toSortedMap()
